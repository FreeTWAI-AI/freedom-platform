...
export function buildProvisionPlan(manifest, envKey) {
  ...
  const steps = [
    { id: 'prerequisites', mutation: false, actor: 'root/operator', action: `Outside this tool: pscale CLI >= ${ps.pscale_cli_min_version} authenticated to the target PlanetScale organization (observed ${ps.pscale_auth_observed.status}, ${ps.pscale_auth_observed.source}); org-level ${db.size} ${db.topology} availability in ${region} read with \`pscale size cluster list\` (not_run). Workers Paid is already authorized (US$${manifest.cloudflare.workers_paid_usd_month}/month).` },
    { id: 'cost-context', mutation: false, actor: 'reviewer', action: `${envKey}: ${cost.environments[envKey].sku} US$${cost.environments[envKey].usd_month}/month list price, billed through Cloudflare at the same price; both environments plus Workers Paid US$${cost.total}/month. Context only, not a gate.` },
