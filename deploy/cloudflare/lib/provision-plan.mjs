...
export function buildProvisionPlan(manifest, envKey) {
  ...
  const steps = [
    { id: 'prerequisites', mutation: false, actor: 'root/operator', action: `Outside this tool: pscale CLI >= ${ps.pscale_cli_min_version} authenticated to the target PlanetScale organization (observed ${ps.pscale_auth_observed.status}; root's latest login observed credential save failed (TOKEN_SAVE_FAILED) and the process ended; root coordinates the fix and no new login is started here), commands with --format json; org-level ${db.size} ${db.topology} availability in ${region} and its regional/org price read with \`pscale size cluster list --org <org> --format json\` (not_run). Workers Paid is already authorized (US$${manifest.cloudflare.workers_paid_usd_month}/month).` },
