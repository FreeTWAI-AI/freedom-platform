...
- As of 2026-09-25 the allowlist is `--target staging` (`https://staging.freetwai.com`) and `--target public` (`https://freetwai.com`). `--target staging-next` and `--target next` no longer resolve. The 2026-09-24 runs above addressed those retired hostnames; they are history, not current targets. The old `verify-staging.mjs`, `verify-public.mjs` and `verify-member-settings.mjs` are not imported or reused. They hard-code old hosts, use demo accounts, create accounts or read member inboxes.

## What the tool can address

| `--target` | Origin | Expected `health.mode` |
| --- | --- | --- |
| `staging` | `https://staging.freetwai.com` | `staging` |
| `public` | `https://freetwai.com` | `public` |

`staging` is the live staging Worker (whole-host Access, Castle demo data, never live member data). `public` is production. `next.freetwai.com` and `staging-next.freetwai.com` were removed on 2026-09-25.

Only these exact strings are accepted, either as the name or as the exact origin with at most one trailing `/`. Everything else is rejected before any request: other hosts, lookalikes, punycode, uppercase, ports, paths, queries, fragments, userinfo and `http:`. There is no bypass flag. The loopback harness target is only reachable from library code in tests. Report filenames use the target name (`staging-account.json`, `staging-messages.json`, `public-account.json`, `public-messages.json`). The report field `target.name` is that same allowlisted name.
...
npx tsx scripts/verify-cloud-candidate.ts plan --target staging --phases health,protocol,assets,anonymous,guild-cache,load

# Read-only gates (default phases: health, protocol, assets, anonymous)
umask 077
FREEDOM_CANDIDATE_ACCESS_FILE=/path/to/private/access.json \
  npx tsx scripts/verify-cloud-candidate.ts execute --target staging --expected-version <version> --expected-release-sha <40-hex commit> \
  > /path/to/private/reports/staging-readonly.json
...
FREEDOM_CANDIDATE_ACCOUNT_FILE=/path/to/private/account.json FREEDOM_CANDIDATE_ACCESS_FILE=/path/to/private/access.json \
  ...
  > /path/to/private/reports/staging-account.json

# Synthetic registration and messaging. No account file; these phases register their own cand-reg members.
# The same command with --target public leaves guild-channel checks not_run (real_history_guarded).
# It does not request /api/v1/me/channels/guild/. The mobile shell still GETs /api/v1/me/channels?kind=guild (summary only).
# --target staging runs those guild-channel checks: the community is Castle demo data, never live member data.
npx tsx scripts/verify-cloud-candidate.ts execute --target staging --expected-version <version> --expected-release-sha <40-hex commit> \
  --phases registration,messages,messages-mobile \
  > /path/to/private/reports/staging-messages.json
...
npx tsx scripts/verify-cloud-candidate.ts execute --target public --expected-version <version> --expected-release-sha <40-hex commit> \
  ...
  > /path/to/private/reports/public-load.json
```
...
The account file for the first command is bound to `https://freetwai.com` and is a dedicated synthetic account, not a real member. `registration`, `messages` and `messages-mobile` do not read an account file. They provision only `cand-reg-*` members. On `public`, live member history stays closed (no guild-channel routes), inboxes of other members are not read, and the run never touches another member. The login foreign-origin case is `staging_origin` `https://staging.freetwai.com`, because `https://freetwai.com` is this target.
...
| anonymous | Session, guild preferences and development status return 401 `login_required` with `no-store` and no session cookie. Login is rejected with `origin_rejected` when Origin is missing, foreign, or a third host: production `https://freetwai.com` (`production_origin`) on `staging`, and `https://staging.freetwai.com` (`staging_origin`) on `public` (no credentials sent). Whole-host Access gate on `staging`. On `public`, unauthenticated `GET /` must reach the app (`anonymous_reaches_app`); there is no whole-host challenge. | none |
...
On `staging` and the local harness the same phase also calls `GET /api/v1/guilds`, `POST /api/v1/guilds/:key/join`, `POST /api/v1/guilds/:key/leave`, `GET /api/v1/me/channels?kind=guild` and `/api/v1/me/channels/guild/:key/messages` and `/read`. Those guild-channel calls stay off `public`.

### `messages-mobile` on `public` (browser shell)
...
`GET /api/v1/me/channels?kind=guild` returns summary fields only: `channel_key`, `name`, `unread_count`, `last_message_at`. The summary has no message body and no sender. No channel history is opened on `public`: nothing requests a path containing `/api/v1/me/channels/guild/`. The member search `GET /api/v1/members` is not called. A window focus re-reads the same summaries and does not open history.
...
- Guild channel history on `public`. `messages` reports `guild_channel: not_run` with reason `real_history_guarded` and does not call those routes. `messages-mobile` does not open one either. The shell's `GET /api/v1/me/channels?kind=guild` is the summary list only. `staging` runs the history checks because that community is Castle demo data, never live member data.
