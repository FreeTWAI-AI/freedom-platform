...
export function createDevelopmentRoutes(readMetrics?:(id:string)=>Promise<GitHubMetrics>){
 const app=new Hono();
 ...
 app.get('/development/skills/:id',async c=>{
   ...
   // Public guidance stays readable during a cache/database outage. It never
   // fetches GitHub or reads member credentials to enrich this static document.
   const metrics=markdown?undefined:await readMetrics?.(id).catch(()=>undefined);
   return markdown?c.text(text):c.html(pageHtml(text.split('\n')[0].slice(2),text,`/development/skills/${encodeURIComponent(id)}.md`,metrics));
 });
