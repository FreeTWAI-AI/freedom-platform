...
export function AdminGitHubSetup({client}:{client:Client}){
  ...
  return <section className="stack"><h2>GitHub 連結</h2>{error&&<p className="banner banner-error" role="alert">{error}</p>}{busy&&<p role="status">正在設定 GitHub…</p>}{status?.configured?<article className="card stack"><h3>站內 Star 已啟用</h3><p>會員連結自己的 GitHub 後，即可加星或取消星星。</p>{status.html_url&&<a href={status.html_url} target="_blank" rel="noopener noreferrer">{status.app_slug??'GitHub App'} ↗</a>}</article>:<article className="card stack"><h3>啟用站內 Star</h3><p>由 FreeTWAI-AI 擁有者建立。只授予 GitHub 星星操作權限。</p><button type="button" className="btn btn-primary" disabled={busy||!status?.setup_available} onClick={()=>void begin()}>建立 GitHub App</button>{status&&!status.setup_available&&<p className="field-hint">GitHub 連結設定尚未啟用。</p>}{error&&<a href="/admin">返回後台</a>}</article>}</section>;
}
