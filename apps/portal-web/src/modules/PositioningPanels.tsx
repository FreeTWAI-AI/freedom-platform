...
export function GuildsPanel({client,onNavigate}:ModulePanelProps) {
  ...
  const [guilds,setGuilds]=useState<GuildSummary[]>([]),[loading,setLoading]=useState(true),[loadError,setLoadError]=useState<string|null>(null),[notice,setNotice]=useState(''),[preferences,setPreferences]=useState<{primary_guild_key:string|null;aggregate_version?:number}|null>(null),[applications,setApplications]=useState<{application_id:string;name:string;profession:string;state:string}[]>([]),[showApply,setShowApply]=useState(false);
  const {mutate,busy,error}=useModuleMutation(client);
  async function load(){setLoadError(null);try{const [g,p,a]=await Promise.all([client.get<{items:GuildSummary[]}>('/guilds/directory'),client.get<{primary_guild_key:string|null;aggregate_version?:number}>('/me/guild-preferences'),client.get<{items:typeof applications}>('/guild-applications')]);setGuilds(g.items);setPreferences(p);setApplications(a.items);}catch(e){setLoadError(failure(e));}finally{setLoading(false);}}
  ...
  const sortedGuilds=guilds.filter(g=>(scope==='all'||g.membership?.state==='active')&&(!search||[g.name,g.purpose,guildMasterLabel(g),...(g.guild_experts??[]).map(expert=>expert.display_name),...g.skill_books.map(book=>book.title)].join(' ').toLocaleLowerCase().includes(search))).sort((a,b)=>(a.is_primary?0:a.membership?.state==='active'?1:2)-(b.is_primary?0:b.membership?.state==='active'?1:2));
  ...
  return <section className="module-panel guilds-panel" aria-label="公會目錄">
    ...
    {!loading&&!loadError&&sortedGuilds.length===0&&<p className="muted" role="status">{search?'找不到符合的公會。':'尚未加入公會。'}<button type="button" className="btn btn-ghost" onClick={()=>{setQuery('');setScope('all');}}>查看全部公會</button></p>}
    <div className="card-grid guild-directory">{sortedGuilds.map(g=><article className={`card guild-card${g.is_primary?' primary-guild':g.membership?.state==='active'?' joined-guild':''}${memberGuild===g.guild_key?' members-open':''}`} key={g.guild_key} aria-label={g.name}>
      <div className="card-head guild-card-topline"><h3>{g.name}</h3>{g.is_primary?<span className="badge">主要公會</span>:g.membership?.state==='active'?<span className="badge">次要公會</span>:null}</div>
      <GuildLeadership masterName={guildMasterLabel(g)} masterId={g.guild_master?.user_id} masterAvatarUrl={g.guild_master?.avatar_url} experts={g.guild_experts}/><p className="guild-purpose">{g.purpose}</p>{typeof g.track_count==='number'&&<small>{g.track_count} 個職業方向</small>}
