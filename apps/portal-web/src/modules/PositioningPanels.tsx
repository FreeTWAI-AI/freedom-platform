...
export function GuildsPanel({client,onNavigate}:ModulePanelProps) {
  ...
  return <section className="module-panel guilds-panel" aria-label="公會目錄">
    ...
    {loading&&<p role="status">正在載入職業公會…</p>}{loadError&&<div role="alert">{loadError}<button onClick={()=>void load()}>重新載入公會</button></div>}{error&&<div role="alert"><p>{error}</p><button className="btn btn-ghost" disabled={busy} onClick={()=>{setEditing(false);void load();}}>重新載入公會</button></div>}{notice&&<p role="status" className="banner status-note">{notice}</p>}
