...
export function GuildsPanel({client}:ModulePanelProps) {
  ...
  return <section className="module-panel" aria-labelledby="guilds-title"><header className="section-heading"><p className="eyebrow">FIND YOUR GUILD</p><h2 id="guilds-title">職業公會</h2><p>找到一起精進專業的人。可以加入多個公會，再選一個作為主力；每個公會都有自己的技能書。</p><button className="btn btn-ghost" onClick={()=>setShowApply(!showApply)}>{showApply?'收起創建申請':'申請創建公會'}</button></header>
    ...
    <div className="card-grid">{[...guilds].sort((a,b)=>(a.is_primary?0:a.membership?.state==='active'?1:2)-(b.is_primary?0:b.membership?.state==='active'?1:2)).map(g=><article className={`card guild-card${g.is_primary?' primary-guild':''}`} key={g.guild_key} aria-label={g.name}><div className="card-head"><h3>{g.name}</h3>{g.is_primary?<span className="badge">主要公會</span>:g.membership?.state==='active'&&<span className="badge">次要公會</span>}</div><p className="guild-master">公會長：{guildMasterLabel(g)}</p><p>{g.purpose}</p><p>可以先做：{g.first_step}</p><small>{g.track_count} 個職業方向 · 公開知識與共同學習免費</small><div className="guild-book-list"><strong>公會技能庫</strong>{g.skill_books.length?g.skill_books.map(book=><SkillBookIntro key={book.book_id} book={book} guildName={g.name} label={book.title}/>):<p className="muted">技能書整理中</p>}</div><div className="actions">{g.membership?.state==='active'&&!g.is_primary&&<button className="btn btn-primary" disabled={busy} onClick={()=>void primary(g)}>設為主要公會</button>}<button type="button" className="btn btn-ghost" disabled={busy||g.is_primary} onClick={()=>void change(g)}>{g.membership?.state==='active'?'退出':'加入'}{g.name}</button></div>{g.is_primary&&<p className="field-hint">若要退出，先將另一個已加入的公會設為主要公會。</p>}</article>)}</div>
