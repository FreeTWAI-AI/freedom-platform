...
const stateLabels:Record<string,string>={draft:'選品草稿',requested:'待供貨商回覆',accepted:'供貨商已確認（內部演練）',declined:'供貨商暫不接受'};
const stateLabel=(state:string)=>stateLabels[state]??'狀態待確認';
...
export function SupplierPanel({client}:ModulePanelProps){
  ...
  const [availability,setAvailability]=useState<'finite'|'manual_confirmation'>('finite');
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      if(created){form.reset();setAvailability('finite');setQuery('');setAvailabilityFilter('all');setNotice('商品與第一版供貨條件已保存，可到「開店與銷售」選品。');await load();}
    }catch(e){setError(e instanceof Error?e.message:'請檢查商品欄位。');}
  }
  return <section className="stack" aria-labelledby="supplier-heading"><ModuleBanner eyebrow="SUPPLY / 把你的好商品帶進來" title="刊登商品，回覆銷售者的供貨請求" headingId="supplier-heading" description="" art="/art/rpg/market-network.webp"/><PreviewNote/>
    ...
    <div className="module-split"><form className="card stack" onSubmit={event=>void submit(event)}><h3>提交實體商品</h3>
      ...
      <div className="form-grid"><label className="field"><span>供貨價（新台幣）</span><input name="net_price" required inputMode="decimal" placeholder="300.00"/></label><label className="field"><span>供貨方式</span><select name="availability" value={availability} onChange={event=>setAvailability(event.target.value as typeof availability)}><option value="finite">有明確庫存</option><option value="manual_confirmation">接單前確認數量</option></select></label></div>
      {availability==='finite'?<label className="field"><span>可供數量（有明確庫存時必填）</span><input name="stock" type="number" required min="0" max="100000000" step="1" defaultValue="0"/></label>:<p className="field-hint">接單前確認數量：銷售者會看到「數量須與供貨商確認」。</p>}
...
function SupplyRequestCard({request:r,busy,onDecide}:{request:SupplyRequest;busy:boolean;onDecide:(decision:'accepted'|'declined',note:string)=>Promise<void>}){
  return <article className="card stack"><div className="card-head"><h4>{r.snapshot.supply.title}</h4><span className="pill">{stateLabel(r.state)}</span></div><p>銷售者：{r.seller_name}</p><SnapshotDetails snapshot={r.snapshot}/>{r.state==='requested'?<form className="stack" onSubmit={event=>{event.preventDefault();const d=new FormData(event.currentTarget);void onDecide(d.get('decision') as 'accepted'|'declined',String(d.get('note')));}}><label className="field"><span>回覆</span><select name="decision" required defaultValue=""><option value="" disabled>請選擇回覆</option><option value="accepted">確認此售價與供貨條件（內部演練）</option><option value="declined">暫不接受</option></select></label><label className="field"><span>給銷售者的說明</span><textarea name="note" required maxLength={1000}/></label><label className="checkbox-row"><input type="checkbox" required/><span>我已核對以上商品、售價與條件；這是內部演練回覆。</span></label><button className="btn btn-primary" disabled={busy}>保存供貨回覆</button></form>:<p>{r.decision_note}</p>}</article>;
}

export function RetailPanel({client,onNavigate}:ModulePanelProps){
