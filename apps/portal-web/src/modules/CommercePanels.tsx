...
export function SupplierPanel({client}:ModulePanelProps){
  ...
  return <section className="stack" aria-labelledby="supplier-heading"><header><p className="eyebrow">把你的好商品帶進來</p><h2 id="supplier-heading">供貨中心</h2><p className="lede">你提供商品與供貨條件，銷售者經營自己的店。每次合作都能看清楚對方的售價與承諾。</p></header><PreviewNote/>
...
export function RetailPanel({client}:ModulePanelProps){
  ...
  return <section className="stack" aria-labelledby="retail-heading"><header><p className="eyebrow">用你的眼光，經營自己的店</p><h2 id="retail-heading">開店與銷售</h2><p className="lede">選擇想賣的商品、設定售價，和供貨商對齊出貨承諾。你的商店由你負責服務買家。</p></header><PreviewNote/>{error&&<p role="alert" className="banner banner-error">{error}</p>}{notice&&<p role="status" className="banner status-note">{notice}</p>}
