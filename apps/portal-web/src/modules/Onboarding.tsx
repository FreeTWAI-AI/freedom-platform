...
export type GuildSummary = { guild_key:string;name:string;purpose:string;first_step:string;track_count:number;guild_master:{user_id?:string;display_name:string}|null;guild_experts?:{user_id:string;display_name:string}[];guild_master_nominee?:{display_name:string;state:'pending'}|null;is_primary:boolean;skill_books:SkillBook[];membership:{state:string;aggregate_version:number}|null };
...
export function SkillBooks({ books }: { books: SkillBook[] }) {
  return <div className="card-grid skill-book-shelf">{books.map(book => <SkillBookCard key={book.id??book.book_id} book={book}/>)}</div>;
}
export function Onboarding({ client, initial, onCompleted, onLogout, optional = false }: {client:PortalClient;initial:OnboardingView;onCompleted:()=>void;onLogout?:()=>void;optional?:boolean}) {
  ...
  const guildChoicesInitialized=useRef(false);
  ...
  return <main className="onboarding-layout guild-onboarding"><header className="onboarding-header"><div className="brand onboarding-brand"><img src="/brand/freedom-workshop.webp" alt="自由工坊" width="1280" height="720"/><p className="eyebrow">FIND YOUR PLACE</p></div>{onLogout&&<button className="btn btn-ghost" disabled={locked} onClick={onLogout}>登出</button>}{optional&&<button className="btn btn-ghost" disabled={locked} onClick={onCompleted}>返回我的定位</button>}</header>
    ...
    {definition&&!finished&&step<4&&<form className="card onboarding-card stack" onSubmit={next}><fieldset disabled={locked} className="stack" style={{border:0,padding:0,margin:0,minWidth:0}} aria-label="定位答案"><div className="onboarding-step-label"><span>定位旅程</span><span>0{step+1} / 05</span></div>
