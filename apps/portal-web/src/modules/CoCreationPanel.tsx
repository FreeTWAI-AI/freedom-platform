...
export function CoCreationPanel({ client, session, onNavigate }: ModulePanelProps) {
  ...
  return <section className="module-panel" aria-labelledby="cocreation-heading">
    <header className="section-heading"><p className="eyebrow">BUILD TOGETHER</p><h2 id="cocreation-heading">一起開發</h2><p>選一個你想幫忙的作品，從具體任務開始。你可以寫程式、測試、做設計或整理文件，也可以帶著 AI 助手一起參與。</p><div className="actions"><button className="btn btn-primary" onClick={() => setShowCreate(!showCreate)}>{showCreate ? '收起邀請表' : '發起共創邀請'}</button><button className="btn btn-ghost" onClick={() => onNavigate?.('opensource')}>看看開源作品</button></div></header>
    <ol className="flow" aria-label="一起開發的步驟"><li>找到想做的事</li><li>在任務頁留言協調</li><li>完成修改並送出 PR</li><li>維護者審查與合併</li></ol>
    ...
    {!loading && !loadError && projects.length === 0 && <div className="card"><h3>從第一個共創邀請開始</h3><p className="muted">把想完成的作品與需要的幫助說清楚，讓夥伴找到參與的起點。</p></div>}
    <div className="card-grid">{projects.map(project => <article className={`card stack${selectedId === project.project_id ? ' primary-guild' : ''}`} key={project.project_id}><p className="eyebrow">{project.source_kind === 'community_pilot' ? '工坊共創起點' : '會員共創邀請'}</p><h3>{project.title}</h3><p>{project.goal}</p><div className="tag-list">{project.help_wanted.map(role => <span className="pill" key={role}>{roleLabel(role)}</span>)}</div><p className="muted">邀請發起：{project.coordinator_name}</p><button className="btn btn-ghost" aria-pressed={selectedId === project.project_id} onClick={() => setSelectedId(project.project_id)}>查看任務與成果</button></article>)}</div>
    {selected && <section className="stack" aria-labelledby="cocreation-project-title"><div className="card stack"><p className="eyebrow">{selected.repository_full_name}</p><h3 id="cocreation-project-title">{selected.title} · 參與方式</h3><p className="cocreation-copy">{selected.contribution_notes}</p><div className="actions"><GitHubLink href={selected.repository_url}>查看專案</GitHubLink><GitHubLink href={`${selected.repository_url}/issues/new`}>提出新任務</GitHubLink>{selected.upstream_url && <GitHubLink href={selected.upstream_url}>認識原作者</GitHubLink>}<button className="btn btn-ghost" disabled={activityLoading} onClick={() => void loadActivity(selected.project_id)}>更新任務與成果</button></div><p className="field-hint">共創邀請的發起人不一定是 Repo 維護者；認領、審查與合併以專案的 GitHub 紀錄為準。</p></div>
