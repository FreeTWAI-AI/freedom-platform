...
export function SquadsPanel({client,session}:ModulePanelProps){
  ...
  async function open(id:string){setDetailError('');try{const data=await client.get<SquadDetail>(`/squads/${id}`);const own=data.members.find(member=>member.user_id===session.user.user_id);setDetail({...data,membership:own?{state:own.state,aggregate_version:own.aggregate_version}:null});}catch(e){setDetailError(e instanceof Error?e.message:'無法載入小隊。');}}
  ...
  return <section className="module-panel squad-panel">
    ...
    <section className="squad-directory" aria-labelledby="squad-directory-heading">
      ...
      <div className="discovery-feedback"><p role="status">顯示 {visibleSquads.length} / {squads.length} 支小隊</p>{filtered&&<button className="btn btn-ghost" onClick={()=>{setQuery('');setKind('all');}}>清除小隊篩選</button>}</div>
      ...
      {!loading&&!loadError&&!visibleSquads.length&&<div className="discovery-empty"><h4>{filtered?'沒有符合條件的小隊':'還沒有小隊'}</h4><button className="btn btn-ghost" onClick={()=>nameInput.current?.focus()}>新增小隊</button></div>}
