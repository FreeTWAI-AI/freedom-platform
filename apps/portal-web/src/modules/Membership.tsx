...
export type MemberCardData={user_id:string;joined_at?:string|null;joined_at_source?:'launch_day'|'registered';avatar_url?:string|null;nickname:string;positioning_title:string|null;primary_guild:GuildRef|null;secondary_guilds:GuildRef[];joined_guilds?:GuildRef[];capabilities:string[];equipment:string[];custom_capabilities?:string[];custom_equipment?:string[];featured_capabilities?:string[];contacts:Partial<Record<'email'|'discord'|'github'|'line',string>>;is_self:boolean;friendship:{state:string;requester_ref?:string;aggregate_version?:number}};
...
export function AccountPanel({client,session,onNavigate}:ModulePanelProps){
  const [account,setAccount]=useState<Account|null>(null),[nickname,setNickname]=useState(''),[contacts,setContacts]=useState<Account['contacts']|null>(null),[loadError,setLoadError]=useState(''),[notice,setNotice]=useState(''),[member,setMember]=useState<MemberCardData|null>(null),[labels,setLabels]=useState<Record<string,string>>({});
