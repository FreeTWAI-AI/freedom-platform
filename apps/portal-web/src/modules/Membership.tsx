...
export type MemberCardData={user_id:string;avatar_url?:string|null;nickname:string;positioning_title:string|null;primary_guild:GuildRef|null;secondary_guilds:GuildRef[];capabilities:string[];equipment:string[];custom_capabilities?:string[];custom_equipment?:string[];featured_capabilities?:string[];contacts:Partial<Record<'email'|'discord'|'github'|'line',string>>;is_self:boolean;friendship:{state:string;requester_ref?:string;aggregate_version?:number}};
...
export function AccountPanel({client,session}:ModulePanelProps){
  ...
  return <section className="module-panel account-panel"><ModuleBanner eyebrow="YOUR MEMBER CARD" title="我的會員名片" description="讓夥伴認識你的方向與能力，聯絡方式由你決定分享給誰。"/>{member&&<MemberCard member={member} labels={labels}/>}
