...
type Account={user_id:string;nickname:string;login_email:string;email_verified:boolean;contacts:Record<'email'|'discord'|'github'|'line',Contact>;aggregate_version:number};
type GuildRef={guild_key:string;name:string};
export type MemberCardData={user_id:string;nickname:string;positioning_title:string|null;primary_guild:GuildRef|null;secondary_guilds:GuildRef[];capabilities:string[];equipment:string[];custom_capabilities?:string[];custom_equipment?:string[];featured_capabilities?:string[];contacts:Partial<Record<'email'|'discord'|'github'|'line',string>>;is_self:boolean;friendship:{state:string;requester_ref?:string;aggregate_version?:number}};
...
export function MemberCard({member,children,labels={}}:{member:MemberCardData;children?:React.ReactNode;labels?:Record<string,string>}){
  ...
  return <article className="card member-card expedition-dossier">
    <div className="member-card-heading">
      <div className="member-avatar" aria-hidden="true">{member.nickname.slice(0,1)}</div>
...
export function MembersPanel({client,session}:ModulePanelProps){
  const [members,setMembers]=useState<MemberCardData[]>([]),[nextOffset,setNextOffset]=useState<number|null>(null),[loadError,setLoadError]=useState(''),[loading,setLoading]=useState(true),[notice,setNotice]=useState(''),[labels,setLabels]=useState<Record<string,string>>({});
  const [friends,setFriends]=useState<{user_id:string;nickname:string;state:string;requester_ref:string;aggregate_version:number}[]>([]);
  const {mutate,busy,error}=useModuleMutation(client);
  async function load(offset=0){setLoadError('');setLoading(true);try{const [m,f]=await Promise.all([client.get<{items:MemberCardData[];next_offset:number|null}>(`/members?limit=20&offset=${offset}`),client.get<{items:typeof friends}>('/friends')]);setMembers(current=>offset?[...current,...m.items]:m.items);setNextOffset(m.next_offset);setFriends(f.items);}catch(e){setLoadError(fail(e));}finally{setLoading(false);}}
  useEffect(()=>{void load();void loadLabels(client).then(setLabels).catch(()=>{});},[client]);
  async function act(userId:string,action:string,version?:number){const result=await mutate(`/friends/${userId}/${action}`,{},version);if(result){setNotice(action==='accept'?'已成為平台好友。':action==='remove'?'好友關係已移除。':'好友邀請已送出，等待對方接受。');await load();}}
  return <section className="module-panel"><ModuleBanner eyebrow="FIND YOUR PEOPLE" title="工坊夥伴" description="找到同路人。每個人的聯絡方式，都依照本人設定的可見範圍顯示。" art="/art/rpg/cooperation-forge.webp"/><Status error={loadError||error} notice={notice}/>{friends.some(f=>f.state==='pending'&&f.requester_ref!==session.user.user_id)&&<section className="card stack"><h3>收到的好友邀請</h3>{friends.filter(f=>f.state==='pending'&&f.requester_ref!==session.user.user_id).map(f=><div className="member-request" key={f.user_id}><strong>{f.nickname}</strong><button className="btn btn-primary" disabled={busy} onClick={()=>void act(f.user_id,'accept',f.aggregate_version)}>接受邀請</button><button className="btn btn-ghost" disabled={busy} onClick={()=>void act(f.user_id,'remove',f.aggregate_version)}>婉拒</button></div>)}</section>}<div className="card-grid member-directory">{members.map(member=><MemberCard key={member.user_id} member={member} labels={labels}>{!member.is_self&&<div className="actions">{member.friendship.state==='accepted'?<><span className="badge">平台好友</span><button className="btn btn-ghost" disabled={busy} onClick={()=>void act(member.user_id,'remove',member.friendship.aggregate_version)}>移除好友</button></>:member.friendship.state==='pending'?<span className="muted">{member.friendship.requester_ref===session.user.user_id?'好友邀請已送出':'對方已邀請你，請從上方接受'}</span>:<button className="btn btn-ghost" disabled={busy} onClick={()=>void act(member.user_id,'request',member.friendship.aggregate_version)}>邀請成為好友</button>}</div>}</MemberCard>)}</div>{loading&&<p role="status">正在尋找工坊夥伴…</p>}{!loading&&!members.length&&!loadError&&<p className="muted">還沒有完成定位的會員。</p>}{nextOffset!==null&&<button className="btn btn-ghost" disabled={loading} onClick={()=>void load(nextOffset)}>查看更多夥伴</button>}{loadError&&<button className="btn" onClick={()=>void load()}>重新載入夥伴</button>}</section>;
}
