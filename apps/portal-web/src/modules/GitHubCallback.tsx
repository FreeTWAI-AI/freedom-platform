...
export function GitHubCallback(){
  ...
  useEffect(()=>{
    ...
    if(!operation.current){
      ...
      operation.current=(async()=>{
        ...
        return /^#[a-z-]+$/.test(result.return_to)?'/'+result.return_to:'/#community';
      })();
    }
  ...
  return <main className="centered"><section className="card stack"><p className="eyebrow">自由工坊</p><h1>連結 GitHub</h1>{error?<><p role="alert">{error}</p><a className="btn btn-primary" href="/#community">返回技能庫</a></>:<p role="status">正在連結…</p>}</section></main>;
}
