...
完整地基與介面照常建立；首批並行驗證有限互助，以及自願作品曝光 → 商機 → 合作 → Seller 外部實收，共用既有 User、Profession、Skill、Opportunity、Squad、WorkItem 與 Result。互助建議例子是「把重複資料整理改為可重用模板／工具」，不是限定產業。作品宣傳與找客戶不必先完成免費互助；未承諾的人類服務不自動由核心團隊補位。

### 15.1 首批共同成果

需求者帶來真實問題與授權樣本，幫助者自選有限投入，學習者可參與測試、文件及使用回饋。每次合作明列雙方實益、最大投入、終止及可重用範圍。約10次真人合作與30天觀察窗是互助採樣的可改預設，不是外展前置門檻或已證明通過線；付費路徑另記真實成交、實收證據與投入工時。
