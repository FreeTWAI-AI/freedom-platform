...
會員在「設定」裡的通知與私訊，資料存放在中央 PostgreSQL（migration `035_member_communications.sql`），由 `modules/member-communications/` 負責。這裡只有站內紀錄：不寄 email、不推播、不連外部服務，也不回填歷史事件。

## API（全部需要會員登入並完成定位）

分頁參數一律是 `limit`（1–50，預設 20）與 `offset`（0–10000，預設 0）；其他查詢參數回 422。列表都是新到舊，同時間以 UUID 由大到小排序。未讀數在同一個資料庫快照內計算，不受目前頁數影響。GET 不會標記已讀。
