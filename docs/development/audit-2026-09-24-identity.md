...
範圍：會員首頁、註冊／登入、定位（含重新探索）、我的名片、工坊夥伴、我的定位、職業公會（含成員視窗）、小隊集合。基準 commit `8338a42`（分支 `audit/identity-20260924`）。本輪只改動 `apps/portal-web/src/modules/` 內負責的元件與專用 CSS，並新增 `tests/e2e/audit-identity.spec.ts`。沒有新增權限、API、資料欄位或未來架構；原始 `freedom-workshop.webp`、作者署名、隱私可見範圍語意，以及名片「社群顯示名稱」和「男／女／外星人／AI」選項都保留。
...
`projects/Freedom-Platform` 的副本停在 **2026-09-17 baseline**。本倉是較新的現行版本：它包含 2026-09-19 的低維運互惠修訂，以及 2026-09-23 公開會員 beta 的註記。README 和現行程式都以本倉版本為準。
...
| Now／Next／Gained | 身分／下一步／已確認事實 | 求助、協助、共同目標，並分列未知與單方回報 | 未實作（實益回報只有 `benefit-observations` 子集）：**未來缺口** |
| 新人定位 | 定位是導航，可略過 | 12 章 2026-09-23 覆寫：新註冊會員**必須**完成定位、選公會、領技能書 | 已實作（`App.tsx` 在 `required&&!completed` 時擋在定位流程）。ORG-02 的 Given「member 跳過定位」寫於 09-17，已被這份覆寫取代，本輪未改 spec |
...
證據目錄在本機 `/tmp/fp-audit-identity/`（未提交）。`probe-before/` 與 `probe-after/` 是各頁桌面 1440px 與手機 390px 的全頁截圖和 `report.json`（heading、溢出、觸控尺寸、輸入字級、重複文字）。`audit-screenshots/` 是新 E2E 的截圖；重跑 `audit-identity.spec.ts` 會寫入 `test-results/audit-identity/`（已被 gitignore）。
...
- `CapabilityTree`：桌面版預設展開所有分類，定位第 3 步桌面頁高約 10,000px。既有 `onboarding-members.spec.ts` 明確要求桌面展開，所以本輪不改；是否在桌面預設收合，需要產品決定。

## 5. 真正的未來缺口，不是回歸

以下都未實作，也沒有被畫面偽裝成已完成；本輪沒有新增這些功能：三個互助入口與 Now／Next／Gained、`participation_terms`、Rank／Office／ModuleStewardship 的 UI、XP 投影、Squad 共同目標週期、結束與移交、ORG-02 的 WorkIntent revision 與 SkillPackage equip，以及 FND-01／02 的 contract tests。另外，定位的「裝備」（工具名稱）和規格裡的 `SkillEquip` 同名不同義，將來實作 SkillEquip 時要換掉其中一個用詞，避免混淆。
