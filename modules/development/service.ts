...
export function developmentMap(){
 return {format:'freedom.development-map/v1',platform_repository:platformRepository,
  ...
  contribution_steps:['先讀原作 Repo 現有的 README、授權與貢獻規則；每個專案的 contribution_target 是預設 PR 目標。','查閱原作 GitHub Issues 與已有 PR，依維護者協作規則確認範圍；工坊任務只作協調來源。','從原作 Fork 到自己的帳號、開獨立分支，保留來源與授權。','完成目標 Repo 的相關測試，向原作提交 PR；說明改變、證據、限制與所有實際貢獻者。'],
  ...
  skill_books:communityCatalog.skill_books.map(book=>({...book,contribution:getSkillCollaboration(book.id)?.contribution??null,guide_url:`/development/skills/${book.id}`,markdown_url:`/development/skills/${book.id}.md`,agent_skill_url:`/development/skills/${book.id}/SKILL.md`,collaboration_url:`/api/v1/skills/${book.id}/collaboration`}))};
}
...
export function skillMarkdown(id:string,editorial?:SkillEditorial|null){
 ...
 return [`# ${book.title}：技能書與協作來源`,(editorial?.summary||guide.beginner.purpose),
  ...
  '## 專案與授權',`原作 Repo：${book.upstream_url}\n從原作建立自己的版本：${book.upstream_url}/fork\n工坊整合參考：${book.repository_url}\n記錄的授權：${book.license_status}`,
...
export function pageHtml(title:string,markdown:string,markdownUrl:string,metrics?:GitHubMetrics,editorial?:SkillEditorial|null,discovery?:SkillDiscoveryBook,intro?:string,origin=LIVE_SITE_ORIGIN){
 ...
 if(book?.guide){
  ...
  const stats='<section class="public-github-social" data-skill-social="'+escape(book.id)+'" data-repository="'+original+'" aria-label="原作者 GitHub 數據"><div class="public-skill-support">'
    +'<a href="'+original+'" target="_blank" rel="noopener noreferrer" aria-label="到 GitHub Star ↗">☆ Stars '+count(metrics?.stargazers_count)+'</a>'
    +'<a href="'+original+'/fork" target="_blank" rel="noopener noreferrer" aria-label="Fork 專案 ↗">Forks '+count(metrics?.forks_count)+'</a>'
...
function collaborationMarkdown(data:SkillCollaboration){
 return [data.intent.summary,`原作 Repo：${data.contribution.url}\n預設 PR 目標：${data.contribution.name}:${data.contribution.default_branch}\n從原作建立自己的版本：${data.contribution.fork_url}\n原作 PR：${data.contribution.pulls_url}`,
 `工坊整合參考：${data.repository.url}\n工坊整合任務專用 Fork：${data.repository.fork_url}\n工坊任務：${data.task_source.issues_url}\n工坊 PR：${data.task_source.pulls_url}\nGitHub Milestones：${data.task_source.milestones_url}`,
 '先閱讀原作現行的貢獻規則。Fork 不會自動回饋原作；修改要以 PR 提交，由原作維護者決定是否合併。工坊整合任務可在工坊審查，另外記錄回饋原作的 PR 或未回送原因。',data.task_source.note,
