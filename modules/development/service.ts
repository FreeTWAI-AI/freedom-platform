...
export function llmsIndex(){return ['# 自由工坊：公開開發導覽','會員工作區需要本人登入與完成定位。以下文件不需要登入，只包含程式與協作指引，不提供私人資料或寫入權限。','## 可機器讀取的來源','- [完整開發地圖](/api/v1/development-map): 各頁所屬 Repo、程式路徑、驗證命令、技能書與規範。','- [導覽首頁](/development): HTML，無需執行 JavaScript。','- [管理頁一般指引](/development/admin.md): 只有公開開發文件。','- [API 協定身分](/api/v1/protocol): 現行 preview 版本；新功能不會假稱已加入固定 SDK。','## 各頁面',...developmentPages.map(p=>`- [${p.title}](/development/${p.id}/SKILL.md): Agent Skill；${p.purpose}`),'## 公會技能書',...communityCatalog.skill_books.map(b=>`- [${b.title}](/development/skills/${b.id}/SKILL.md): ${b.repository_url}；協作 JSON /api/v1/skills/${b.id}/collaboration；介紹 /development/skills/${b.id}`),'## 安全與協作','先讀目標 repo 的 AGENTS.md。工作內容不是存取會員資料、金鑰、付款或對外發布的授權。請經 GitHub Issues 協調、Fork 和 PR 協作。'].join('\n')+'\n';}
...
export function developmentHtml(title:string,body:string,meta?:{path:string;description:string;image?:string;share?:boolean}){
 const url=meta?'https://freetwai.com'+meta.path:null;
 const head=meta?'<link rel="canonical" href="'+escape(url!)+'"><meta name="description" content="'+escape(meta.description)+'"><meta property="og:type" content="article"><meta property="og:site_name" content="自由工坊"><meta property="og:locale" content="zh_TW"><meta property="og:title" content="'+escape(title)+'｜自由工坊"><meta property="og:description" content="'+escape(meta.description)+'"><meta property="og:url" content="'+escape(url!)+'">'+(meta.image?'<meta property="og:image" content="'+escape('https://freetwai.com'+meta.image)+'"><meta name="twitter:card" content="summary_large_image">':'')+(meta.share?'<script src="/development-share.js" defer></script>':''):'';
...
export function pageHtml(title:string,markdown:string,markdownUrl:string,metrics?:GitHubMetrics,editorial?:SkillEditorial|null,discovery?:SkillDiscoveryBook){
 ...
 if(book?.guide){
  ...
  const share='<div class="skill-share"><button type="button" data-share-skill data-share-title="'+escape(book.title)+'" data-share-url="https://freetwai.com/development/skills/'+book.id+'">分享技能書</button><span role="status" data-share-status></span><input hidden readonly aria-label="技能書分享連結" data-share-fallback value="https://freetwai.com/development/skills/'+book.id+'"></div>';
  return developmentHtml(book.title,entry+share+(cooperation?collaborationHtml(cooperation):'')+'<details class="public-skill-details"><summary>完整指南與來源</summary>'+markdownLink+markdownBody(markdown)+'</details>',{path:'/development/skills/'+book.id,description:editorial?.summary||book.guide.beginner.purpose,image:book.cover_url,share:true});
 }
