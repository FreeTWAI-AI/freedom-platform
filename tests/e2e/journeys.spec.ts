...
test('member completes work, reviewer accepts it, result survives reload and login',async({page})=>{
  ...
  await page.getByRole('button',{name:'認領這張工作',exact:true}).click();
  await page.getByRole('button',{name:'開始進行',exact:true}).click();
  await page.getByLabel('提交摘要',{exact:true}).fill('已完成可重用範例與三個步驟。');
  await page.getByLabel(artifactLabel).fill('artifact:browser-result-v1');
  await page.getByRole('button',{name:'提交成果',exact:true}).click();
  await expect(page.getByText('已提交，等待被指派的回饋者處理。提交會進入回饋佇列。')).toBeVisible();
  await switchAccount(page,'reviewer@local.test');
  await page.getByRole('button',{name:'開始回饋',exact:true}).click();
  await page.getByLabel('回饋說明',{exact:true}).fill('範例能重跑，符合完成條件。');
  await page.getByRole('button',{name:'送出決定',exact:true}).click();
  await expect(page.getByText('沒有待回饋項目',{exact:true})).toBeVisible();
  await switchAccount(page,'maker@local.test');
  await expect(page.getByText('artifact:browser-result-v1',{exact:true})).toBeVisible();
  await page.reload();await expect(page.getByText('artifact:browser-result-v1',{exact:true})).toBeVisible();
...
test('showcase and opportunity become a bilateral cooperation and attributed receipt report',async({page})=>{
  ...
  await switchAccount(page,'client@local.test');await page.getByRole('button',{name:'合作紀錄',exact:true}).click();
  await page.getByRole('button',{name:'同意這份合作',exact:true}).click();
  await expect(page.getByText('已同意',{exact:true})).toBeVisible();
  await switchAccount(page,'maker@local.test');await page.getByRole('button',{name:'合作紀錄',exact:true}).click();
  ...
  await page.reload();await page.getByRole('button',{name:'合作紀錄',exact:true}).click();
  await expect(page.getByText(/雙方確認（未核對銀行）·/)).toBeVisible();
