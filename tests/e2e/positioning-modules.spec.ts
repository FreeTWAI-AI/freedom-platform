...
test('member saves their direction, returns after reload and freely joins a vertical Guild',async({page})=>{
  ...
  await page.getByLabel('這些是我目前的想法，我確認保存').check();await page.getByRole('button',{name:'保存我的方向卡',exact:true}).click();
  await expect(page.getByText('方向卡已保存。你可以修改，也可以開始下面的小步。')).toBeVisible();
  await page.reload();await page.getByRole('button',{name:'我的定位',exact:true}).click();await expect(page.getByLabel('我現在想完成的事')).toHaveValue('找到願意一起販售茶葉的夥伴');
