...
test('admin appointment, access sync, revocation and reactivation use real isolated records',async({page,browser})=>{
  ...
  try{
    ...
    await expect(targetPage.getByRole('navigation',{name:'平台管理選單'})).toBeVisible();
    expect((await pool.query('SELECT active FROM users WHERE user_id=$1',[targetId])).rows[0].active).toBe(true);
    // The intended guild member is beyond the general member directory's first
    // 25 rows. The guild-scoped candidate query must still find them directly.
    ...
    await pool.query("INSERT INTO positioning_profession_memberships(membership_id,community_id,user_id,guild_key,state) VALUES($1,$2,$3,$4,'active')",[randomUUID(),DEMO_COMMUNITY,targetId,guildKey]);
    ...
    await expect(choice).toBeVisible();await picker.getByRole('searchbox',{name:'搜尋公會長人選',exact:true}).fill('Zulu');await picker.getByRole('button',{name:'查詢人選',exact:true}).click();await expect(choice).toBeVisible();
    ...
    expect(appointmentAudit).toHaveLength(1);expect(appointmentAudit[0].after_state.user_id).toBe(targetId);expect(appointmentAudit[0].reason).toBe('本人同意帶領讀書會與活動安排');
