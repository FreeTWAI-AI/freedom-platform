...
test('skill maintainers publish structured tasks and milestones without a raw JSON editor',async({page})=>{
  ...
  await login(page);await page.getByRole('button',{name:'公會與技能管理',exact:true}).click();await page.getByRole('button',{name:'技能書編輯',exact:true}).click();await expect(page.getByRole('textbox',{name:'一句話摘要',exact:true})).toHaveValue(draft.summary);
...
test('retry after an unknown announcement result retains the idempotency key',async({page})=>{
  ...
  await login(page);await page.getByRole('button',{name:'公會與技能管理',exact:true}).click();await page.getByLabel('公告標題',{exact:true}).fill('安排下一場共創');await page.getByLabel('公告內容',{exact:true}).fill('帶一份可以公開的合成測試素材。');await page.getByRole('button',{name:'保存公告',exact:true}).click();await expect(page.getByRole('alert')).toContainText('尚未確認');await expect(page.getByLabel('公告標題',{exact:true})).toHaveValue('安排下一場共創');await page.getByRole('button',{name:'保存公告',exact:true}).click();await expect(page.locator('.guild-workspace').getByRole('status')).toContainText('公告已保存');expect(keys).toHaveLength(2);expect(keys[0]).toBeTruthy();expect(keys[1]).toBe(keys[0]);expect(items).toHaveLength(1);
});
