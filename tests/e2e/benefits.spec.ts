...
test('optional benefit reports load only when opened, persist privately and remain available after acceptance',async({page,browser,baseURL})=>{
 ...
 try{
  ...
  await card.getByRole('button',{name:'保存我的回報',exact:true}).click();await expect(card.getByRole('status')).toContainText('已保存你的回報');
  ...
  await ownerCard.getByRole('button',{name:'保存我的回報',exact:true}).click();await expect(ownerCard.getByRole('status')).toContainText('已保存你的回報');
  ...
  await card.getByLabel('想補充的經驗（選填）').fill('');await card.getByRole('button',{name:'更新我的回報',exact:true}).click();await expect(card.getByRole('status')).toContainText('已保存你的回報');
