...
test('onboarding recommendations show the master above three separate expert rows and retain guild selection',async({page})=>{
  ...
  await page.goto('/');await page.getByRole('button',{name:'建立帳號',exact:true}).click();await page.getByLabel('社群顯示名稱',{exact:true}).fill('公會人物列測試');await page.getByLabel('電子郵件',{exact:true}).fill(`guild-people-${randomUUID()}@example.test`);await page.getByLabel('密碼',{exact:true}).fill('freedom-leadership-test-2026');await page.getByRole('button',{name:'註冊並開始定位',exact:true}).click();
