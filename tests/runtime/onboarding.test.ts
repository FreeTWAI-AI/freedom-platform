...
test('guild directory shows only active pending nominees in its community and never exposes their email',async()=>{
 ...
 rows=(await request('/guilds/directory',member)).data.items;
 const appointed=rows.find((g:any)=>g.guild_key==='guild_security');assert.equal(appointed.guild_master.display_name,member.user.display_name);assert.equal(appointed.guild_master.user_id,member.user.user_id);assert.deepEqual(Object.keys(appointed.guild_master).sort(),['avatar_url','display_name','user_id']);assert.equal(appointed.guild_master_nominee,null);
