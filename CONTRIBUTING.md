...
新增瀏覽器案例請從 `tests/e2e/fixtures.ts`（同目錄使用 `./fixtures.js`）匯入 `test`／`expect`。Playwright 單一 worker 使用每次新建的獨立 schema；fixture 只在案例之間重設該 schema 的登入限流計數，避免其他案例用完同一個測試來源的額度。每個案例內仍保留正式限流行為，不使用對外重設入口或產品繞過開關。
